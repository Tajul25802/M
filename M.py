#Gift By : wasimmoulik008
#Github : https://github.com/wasimmoulik008
#Fb Id    : wasim
#Auto Share Sc Gift 
###----------[ IMPORT LIBRARY ]---------- ###
...
██ ███ ██ ██   ██      ██ ██ ██  ██  ██ 
 ███ ███  ██   ██ ███████ ██ ██      ██ '''
 cetak(nel(banner,title=f'{P2} {K2}[ {H2}W{H2}A{H2}S{H2}A{H2}I{B2}M{B2}U{B2}T{B2}O {K2}S{K2}H{K2}A{K2}R{K2}E {H2}]',subtitle_align='left',padding=1,style='blue'))
	
###----------[ LOGIN MINU ]----------###	
def login():
	os.system("clear")
	cetak(nel(f'\n \t {H2}  FB AUTO SHARE TOOLS BRO \n\n \t\t {H2}~[{K2} W{K2}A{K2}S{K2}I{K2}M {H2}]~',title=f'{P2}  {H2}[{K2} W{K2}A{K2}S{K2}I{K2}M {K2}T{K2}E{K2}R{K2}M{K2}U{K2}X {K2}H{K2}E{K2}L{K2}P{K2}I{K2}N{K2}G {K2}Z{K2}O{K2}N{K2}E {H2}]', width=54,padding=(1,4),style='blue'))
	...
	cookie = input(f"{P}   └──> : {H}")
	try:
		data = ses.get("https://business.facebook.com/business_locations", heades = {"user-agent": "Mozilla/5.0 (Linux; Android 8.1.0; MI 8 Build/OPM1.171019.011) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.86 Mobile Safari/537.36","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","content-type":"text/html; charset=utf-8"}, cookies = {"cookie":cookie})
...
{B2} Current Date    :{H2} {hari}, {tanggal}''',title=f'{P2} {H2}[ {K2}Cookie Id Information {H2}]',subtitle_align='left',padding=1,style='blue'))
	...
	cetak(nel(f'{H2} W{H2}A{H2}S{H2}I{H2}M {B2}F{B2}B {O2}A{O2}U{O2}T{O2}O {N2}S{N2}H{N2}A{N2}R{N2}E {K2}S{K2}T{K2}A{K2}R{K2}T',subtitle=f'{P2}┌─',subtitle_align='left',width=29,padding=0,style='blue'))
	...
	try:
		...
		for x in range(jumlah):
			...
			if "id" in post:
				...
				print(f'{P}\r   └──>{H} {bas} {K} W{K}A{K}A{K}S{K}I SEND {H}[{n}]{P}{K} AUTO SHARE {H} SUCSSES{H} ',end='');sys.stdout.flush()
			else:
				print("\n")
